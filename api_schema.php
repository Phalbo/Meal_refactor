...
function initSchema(PDO $pdo): void
{
    ...
    // ── Seed casalinghi in nutrition_db ────────────────────────────────────────

    $casalinghi = [
        'carta igienica','fazzoletti','carta da cucina','sacchetti spazzatura',
        'sacchetti freezer','pellicola trasparente','alluminio',
        'detersivo piatti liquido','detersivo lavastoviglie','sale lavastoviglie',
        'brillantante lavastoviglie','detersivo lavatrice','ammorbidente',
        'candeggina','anticalcare','detersivo bagno','detersivo pavimenti',
        'sgrassatore','vetri spray','spugne','pagliette','guanti gomma',
        'scopino WC','deodorante WC','profumatore ambiente','sapone mani liquido',
        'shampoo','balsamo capelli','bagnoschiuma','sapone solido','dentifricio',
        'spazzolino','filo interdentale','collutorio','rasoi','schiuma da barba',
        'cotone idrofilo','cerotti','paracetamolo','ibuprofene','aspirina',
        'disinfettante','garze','bende','termometro','batterie AA','batterie AAA',
        'lampadine','nastro adesivo','detersivo multiuso',
        'spray disinfettante superfici','salviettine umidificate','assorbenti',
        'pannolini','salviettine bebè','crema mani','acqua ossigenata',
    ];
    $insCasa = $pdo->prepare(
        "INSERT OR IGNORE INTO nutrition_db (name, kcal_100g, zone) VALUES (?, 0, 'casalinghi')"
    );
    foreach ($casalinghi as $c) {
        $insCasa->execute([trim($c)]);
    }

    // ── Seed da file esterni (idempotente) ─────────────────────────────────────
    try { seedIfEmpty($pdo); } catch (\Throwable $e) {}
}

// ── Rilevamento zona statico (usato solo nel seed) ─────────────────────────────
function detectZoneStatic(string $name): string
{
    ...
    $map = [
        'ortofrutta' => [
            ...
            'finocchio','carciofi','asparagi','fagiolini','piselli',
        ],
        'pane'       => [
            'pane','cornett','crackers','grissini','fette biscottate','piadina',
            'baguette','focaccia','ciabatta',
        ],
        'macelleria' => [
        ...
        'latticini'  => [
            ...
            'provolone','emmental','brie','camembert',
        ],
        'scaffali'   => [
            ...
            'ketchup','soia','cocco','uvetta','pinoli','mandorle','noci',
        ],
        'bevande'    => [
            'acqua','vino','birra','succo','aranciata','coca','limonata',
            'tè','tisana','camomilla','latte di','bevanda',
        ],
        'surgelati'  => [
            'surgelat','gelato','piselli surgelati','bastoncini','pizzette surgelate',
            'crocchette','spinaci surgelati','misto mare surgelato',
        ],
        'casalinghi' => [
            'carta igienica','detersivo','shampoo','dentifricio','candeggina',
            'ammorbidente','spugne','sacchetti','pellicola','alluminio',
        ],
    ];
...
function parseXlsxRecipes(string $path): array
{
    ...
    // legge shared strings (celle di tipo 's')
...
function seedIfEmpty(PDO $pdo): void
{
    ...
    // 1. nutrition_db da nutrition.json
    $jsonPath = __DIR__ . '/nutrition.json';
    if (file_exists($jsonPath)) {
        $already = (int)$pdo->query(
            "SELECT COUNT(*) FROM nutrition_db WHERE zone != 'casalinghi'"
        )->fetchColumn();
        if ($already === 0) {
    ...
    // 3. Ricette di sistema da ricette.xlsx
    if ((int)$pdo->query("SELECT COUNT(*) FROM meals WHERE is_system=1")->fetchColumn() === 0) {
        ...
        $insMeal = $pdo->prepare(
            "INSERT OR IGNORE INTO meals
                (name, emoji, category_id, cal_per_adult, notes, is_system)
             VALUES (?,?,?,?,?,1)"
        );
        $insIng = $pdo->prepare(
            "INSERT INTO meal_ingredients
                (meal_id, name, quantity, unit, zone)
             VALUES (?,?,?,?,?)"
        );
        foreach ($recipes as $r) {
            $insMeal->execute([
                $r['name'], $r['emoji'], $r['catId'],
                $r['kcal'], $r['note'] ?: null,
            ]);
            ...
            foreach ($r['ings'] as $ing) {
                ...
                $zone = detectZoneStatic($ing['name']);
...
// ── Ricette hardcoded di fallback (quando ricette.xlsx non è disponibile) ──────
function getHardcodedMeals(): array
{
    return [
        ...
        ['name'=>'Pasta aglio olio peperoncino','emoji'=>'🧄', 'catId'=>2, 'kcal'=>420, 'note'=>'10 minuti',
        ...
        ['name'=>'Salmone al forno',            'emoji'=>'🐟', 'catId'=>3, 'kcal'=>350, 'note'=>'',
         'ings'=>[['name'=>'Salmone fresco','quantity'=>600,'unit'=>'g'],['name'=>'Limone','quantity'=>1,'unit'=>'pz'],['name'=>'Olio EVO','quantity'=>2,'unit'=>'cucchiai'],['name'=>'Erba cipollina','quantity'=>1,'unit'=>'mazzo']]],
        ...
        ['name'=>'Salsicce e friarielli',       'emoji'=>'🌿', 'catId'=>3, 'kcal'=>480, 'note'=>'Tipico napoletano',
        ...
        ['name'=>'Wurstel e patatine',          'emoji'=>'🌭', 'catId'=>3, 'kcal'=>480, 'note'=>'Da bambini',
